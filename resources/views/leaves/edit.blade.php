                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('leaves.update', $leave->id) }}">
                        ...
                        <div>
                            ...
                            <x-text-input id="type" name="type" type="text" class="mt-1 block w-full" :value="old('type', $leave->type)" required autofocus />
                            <x-input-error :messages="$errors->first('type')" for="type" class="mt-2"  />
                        </div>
                        <div class="mt-4">
                            <x-input-label for="reason" :value="__('Leave Reason')" />
                            <textarea id="reason" name="reason" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" rows="4">{{ old('reason', $leave->reason) }}</textarea>
